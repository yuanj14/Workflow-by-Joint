      ...
      defaultLink: function (elementView, magnet) {
        return new joint.shapes.standard.Link({
          router: {
            //优化后的正交曲线模式
            name: 'manhattan',
            args: { startDirections: ['right'], endDirections: ['left'] },
          },
          connector: {
            // name: 'jumpover'
            name: 'normal',
          ...
          attrs: {
            line: { stroke: 'red', strokeDasharray: '4 2', connection: true },
          },
        })
      },
      interactive: { linkMove: true },
      // snapLinks: {radius: 30},
      defaultConnectionPoint: { name: 'boundary', args: { offset: -4.5 } },
      // 验证连线是否允许
      validateConnection: function (
        ...
        if (end === 'target') {
          ...
          if (
            ...
            magnetS.getAttribute('port-group').indexOf('in') >= 0 &&
            magnetS.getAttribute('port-group').indexOf('e') < 0
          ) {
            return (
              magnetT &&
              magnetT.getAttribute('port-group') &&
              magnetT.getAttribute('port-group').indexOf('out') >= 0 &&
              magnetT.getAttribute('port-group').indexOf('e') < 0
            )
          }
          // 连线起点是普通"out"类型时，连线目标是普通"in"类型连接点
            ...
            magnetS &&
            magnetS.getAttribute('port-group') &&
            magnetS.getAttribute('port-group').indexOf('out') >= 0 &&
            magnetS.getAttribute('port-group').indexOf('e') < 0
          ) {
            return (
              magnetT &&
              magnetT.getAttribute('port-group') &&
              magnetT.getAttribute('port-group').indexOf('in') >= 0 &&
              magnetT.getAttribute('port-group').indexOf('e') < 0
            )
          }
